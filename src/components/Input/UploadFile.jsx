        ...
        <div className='flex flex-col w-full'>
            <div className="h-36 w-full overflow-hidden relative shadow-md items-center rounded-md cursor-pointer border-3 border-dashed border-borderColor-main">
                ...
                <div className="h-full w-full bg-transparent absolute z-1 flex justify-center items-center top-0">
                    <div className="flex flex-col">
                        ...
                        <h6 className="text-base text-primary-normal font-semibold text-center mb-1"><span className='text-secondary-main'>Upload a file</span> or drag and drop</h6>
                        <p className="text-primary-light text-sm text-center">MP4, AVI, MOV, MKV, M4V, WEBM up to 80MB</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
                {files && files.map((file, key) => {
                    return (
                        <div key={key} className='w-full h-16 flex items-center justify-between rounded p-3'>
                            <div className="flex flex-row items-center gap-2">
                                <span className="truncate w-44">{file.name}</span>
                            </div>

                            <div className='flex' onClick={() => { removeImage(file.name) }} >
